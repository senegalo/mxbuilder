...
(function() {
    ...
    function attach( editor ) {
        ...
        var layout = function layout( evt ) {
            ...
            function changeMode( newMode ) {
                ...
                switch ( newMode ) {
                    case 'top':
                        //account for the editor padding 8px
                        updatePos( 'absolute', 'top', editorPos.y - 8 );
                        break;
                    case 'pin':
                        updatePos( 'fixed', 'top', pinnedOffsetY );
                        break;
                    case 'bottom':
                        //account for the editor padding
                        updatePos( 'absolute', 'top', editorPos.y + ( editorRect.height || editorRect.bottom - editorRect.top ) - spaceHeight + 8);
                        break;
                }
            ...
            //initilize left or right position depending on how close are we to
            //the borders of the screen
            var editorPos = editable.getDocumentPosition();   
            if(editorPos.x < 200){
                floatSpace.setStyle("left",(editorRect.right ||editorPos.x+editorRect.width)+20+"px");
                floatSpace.setStyle("margin-left","0px");
            } else {
                floatSpace.setStyle("left",editorPos.x - spaceRect.width - 20+"px");
            }
            ...
            // Pin the space element while page scrolls down to pull it off the view port.
                ...
                else if ( editorRect.bottom > spaceHeight + pinnedOffsetY )
                    changeMode( 'pin' );
            }
            ...
            if ( resize ) {
                ...
                if ( mode == "bottom" && editorRect.bottom < spaceHeight + pinnedOffsetY )
                    changeMode( 'pin' );
            }
        };
